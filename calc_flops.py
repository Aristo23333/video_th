...
#改为针对视频的throughput
@torch.no_grad()
def throughput_video(clips, model):
    model.eval()
    ...
    print(f"Single image inference time: {(tic2 - tic1) / (30*batch_size) * 1000:.3f} ms")    
    #返回clip的throughput和inference time
    return 30 * batch_size / (tic2 - tic1), (tic2 - tic1) / (30*batch_size) * 1000
...
def get_flops(model, img_size=224, show_detail=False):
    ...
    print('total flops (G):', sum(sum(i) for i in [conv_flops, linear_flops, mamba_flops]) / 1e9)